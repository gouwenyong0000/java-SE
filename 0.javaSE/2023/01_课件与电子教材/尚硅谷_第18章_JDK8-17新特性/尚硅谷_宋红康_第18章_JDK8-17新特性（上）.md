...
`collect(Supplier<R> supplier, BiConsumer<R, ? super T> accumulator, BiConsumer<R, R> combiner)` 方法接收三个核心参数，它们共同定义了收集过程的各个阶段：

1. **`Supplier<R> supplier` (提供者)**：
   - **作用:**  负责**创建一个新的、空的、可变的结果容器**。
   - **类型:**  `Supplier<R>` 是一个函数式接口，它只包含一个方法 `get()`，`get()` 方法**不接收任何参数，但返回一个类型为 `R` 的对象**。
   - **何时调用:**  在 `collect` 操作的**开始阶段**，以及在**并行流的并行处理中**，每个线程都会调用 `supplier.get()` 来创建一个独立的容器。
   - 示例:
     - 如果您想将元素收集到一个 `ArrayList` 中，`supplier` 可以是 `ArrayList::new` (方法引用) 或者 `() -> new ArrayList<>()` (Lambda 表达式)。
     - 如果您想将元素连接成一个 `StringBuilder`，`supplier` 可以是 `StringBuilder::new` 或者 `() -> new StringBuilder()`。
2. **`BiConsumer<R, ? super T> accumulator` (累加器)**：
   - **作用:**  负责将**流中的元素逐个添加到结果容器中**。
   - **类型:**  `BiConsumer<R, ? super T>` 是一个函数式接口，它包含一个方法 `accept(R container, ? super T element)`，`accept()` 方法**接收两个参数，没有返回值**。
     - **第一个参数 `R container`:**  是**当前的结果容器** (由 `supplier` 创建)。
     - **第二个参数 `? super T element`:** 是**流中当前正在处理的元素**。 `? super T` 表示元素类型可以是 `T` 或 `T` 的父类型，这提供了更大的灵活性。
   - **何时调用:**  ==对于流中的**每个元素**，`accumulator.accept(container, element)` 都会被调用，将元素添加到 `container` 中。==
   - 示例:
     - 如果容器是 `ArrayList`，`accumulator` 可以是 `List::add` (方法引用) 或者 `(list, element) -> list.add(element)` (Lambda 表达式)。
     - 如果容器是 `StringBuilder`，`accumulator` 可以是 `StringBuilder::append` 或者 `(sb, element) -> sb.append(element)`。
3. **`BiConsumer<R, R> combiner` (组合器)**：
   - **作用:**  在**并行流**的情况下，当多个线程并行处理流的不同部分并将结果收集到各自的容器后，`combiner` 负责**将这些独立的容器合并成一个最终的结果容器**。
   - **类型:**  `BiConsumer<R, R>` 是一个函数式接口，它包含一个方法 `accept(R container1, R container2)`，`accept()` 方法**接收两个类型为 `R` 的容器，没有返回值**。
     - **参数 `R container1`:**  是**第一个容器** (通常是累积结果的目标容器)。
     - **参数 `R container2`:**  是**第二个容器** (通常是需要被合并的容器)。
   - **何时调用:**  ==只有在**并行流**中，`combiner` 才会被调用。在**顺序流**中，`combiner` 不会被调用==。
   - 示例:
     - 如果容器是 `ArrayList`，`combiner` 可以是 `List::addAll` 或者 `(list1, list2) -> list1.addAll(list2)`。
     - 如果容器是 `StringBuilder`，`combiner` 可以是 `StringBuilder::append` 或者 `(sb1, sb2) -> sb1.append(sb2)`。

**工作流程:**

1. **创建容器:**  `collect` 操作首先调用 `supplier.get()` 创建一个或多个空的结果容器。在顺序流中，只会创建一个容器；在并行流中，每个线程可能会创建一个容器。
2. **累积元素:**  对于流中的每个元素，`collect` 操作会调用 `accumulator.accept(container, element)` 将元素添加到容器中。在并行流中，不同的元素可能会被添加到不同的容器中，并行进行累积。
3. **合并容器 (仅并行流):**  如果流是并行流，`collect` 操作会在所有元素处理完成后，使用 `combiner.accept(container1, container2)` 将多个容器合并成一个最终的容器。合并操作也是并行进行的。
4. **返回结果:**  最终，`collect` 操作会返回合并后的结果容器 (在顺序流中，就是最初创建的容器)。



**示例代码**： 使用 `collect(supplier, accumulator, combiner)` 连接字符串

```java
public static void main(String[] args) {
        List<String> words = Arrays.asList("Hello", ", ", "World", "!");

        // 使用 collect(supplier, accumulator, combiner) 连接字符串
        StringBuilder sentence = words.stream()
                .collect(
                        StringBuilder::new, // supplier: 创建 StringBuilder 容器
                        StringBuilder::append, // accumulator: 将字符串添加到 StringBuilder
                        StringBuilder::append  // combiner: 合并两个并行流中的容器 StringBuilder
                );

        System.out.println("连接后的字符串: " + sentence.toString()); // 输出: 连接后的字符串: Hello, World!
    }
```

> **代码解释:**
>
> - `StringBuilder::new`:  `supplier`，使用 `StringBuilder` 的构造方法创建一个新的 `StringBuilder` 对象作为容器。
> - `StringBuilder::append`:  `accumulator` 和 `combiner` 都使用了 `StringBuilder::append` 方法。
>   - **`accumulator`:**  将流中的每个字符串元素 `element` 追加到当前的 `StringBuilder` 容器中。
>   - **`combiner`:**  **在并行流中**，如果需要合并两个 `StringBuilder` 容器 `sb1` 和 `sb2`，则将 `sb2` 的内容追加到 `sb1` 的末尾。

**何时使用 `collect(supplier, accumulator, combiner)` ?**

- **自定义收集逻辑:**  当您需要执行一些**标准 `Collector` 无法满足的自定义收集逻辑**时，例如将元素收集到自定义的数据结构中，或者执行一些复杂的累积和合并操作。
- **理解底层原理:**  如果您想更深入地**理解 `collect` 操作的底层原理**，以及可变归约的过程，使用这种形式的 `collect` 可以帮助您更好地掌握细节。
- **某些特定场景:**  在一些特定的性能敏感场景下，您可能需要根据具体情况，手动实现 `supplier`, `accumulator`, 和 `combiner` 来进行更精细的性能调优。

**与 `collect(Collector)` 的比较:**

- **`collect(Collector)`:**  更**高级、更简洁**，通常用于执行常见的收集操作，例如 `toList()`, `toSet()`, `toMap()`, `groupingBy()`, `joining()` 等。 Java 提供了丰富的预定义 `Collector`，可以满足大多数常见的需求。
- **`collect(supplier, accumulator, combiner)`:**  更**底层、更灵活**，允许您完全自定义收集过程，但代码也更冗长，需要更多地关注细节。
