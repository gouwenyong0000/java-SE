...
| **方   法**             | **描   述**                                                  |
| ----------------------- | ------------------------------------------------------------ |
| **filter(Predicatep)**  | 接收  Lambda ， 从流中排除某些元素  过滤                     |
...
```java
使用提供的标识值和关联累积函数对此流的元素执行缩减，并返回缩减后的值。这相当于：
T result = identity; 
...
identity – 累积函数的身份值 
accumulator – 用于组合两个值的关联、不干扰、无状态函数
...
```java
//方式1 灵活的基类操作 ：以下简称方式1。 ## 注意三个参数的范型。 
 <R> R collect(Supplier<R> supplier,//创建一个可变结果容器               提供者
                  BiConsumer<R, ? super T> accumulator,//加入到容器操作  累加器
                  BiConsumer<R, R> combiner);//合并容器结果              组合器
//第一个为收集结果的容器,第二个为每个结果如何添加到容器,第三个为容器如何合并
等价于
R result = supplier. get(); 
for (T element : this stream)     
	accumulator. accept(result, element); 
result.accept(result1)
return result;
...
//方式1官方案例：
 List<String> asList = stringStream.collect(ArrayList::new, ArrayList::add,ArrayList::addAll);
    等价于
    List<String> asList = listData.stream().collect(
            () -> new ArrayList<>(),   //创建可变的接收容器
            (list, x) -> list.add(x),  //容器添加结果
            (list1, list2) -> list1.addAll(list2)   //合并容器结果

    );
//案例2------------------------------------
StringBuilder collect = stringStream.collect(
    StringBuilder::new, 
    StringBuilder::append,
    StringBuilder::append);
等价于
String string = strs.stream().collect(
                () -> new StringBuilder(),//第一个参数,用StringBuilder
                (c, x) -> c.append(x), //第二个参数,如何添加到容器
                (r1, r2) -> r1.append(r2)).toString();//第三个参数,容器如何合并

//案例3   Map收集数据------------------------------------
HashMap<Long,String> collect = list.stream().collect(
    ()->new HashMap<Long, String>(),//创建可变结果容器
    (h,x)->{
    h.put(x.getId(),x.getName());
    },//加入容器操作
    HashMap::putAll);//容器合并操作
```



```java
 //方式2 封装了常用的转换操作方式 ：以下简称方式2  如toList toSet
// Collector封装了用作collect(Supplier, BiConsumer, BiConsumer)参数的函数
 <R, A> R collect(Collector<? super T, A, R> collector); 
