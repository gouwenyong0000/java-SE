...
另外，<u>**使用this访问属性和方法时，如果在本类中未找到，会从父类中查找**</u>。这个在继承中会讲到。
...
>   - 上面要求推论：在类的一个构造器中，最多只能声明一个"this(参数列表)"
...
- <u>当子类对象被创建时，在堆中给对象申请内存时，就要看子类和父类都声明了什么实例变量，这些实例变量都要分配内存</u>。
- 当子类对象调用方法时，<u>编译器会先在子类模板中看该类是否有这个方法，如果没找到，会看它的父类甚至父类的父类是否声明了这个方法，遵循`从下往上`找的顺序，找到了就停止，一直到根父类都没有找到，就会报编译错误</u>。
...
方法的重载：同一个类中，方法名相同，形参列表不同。不看返回值类型。 “两同一不同”
...
- 尤其当子父类出现同名成员时，可以用super表明调用的是父类中的成员【`属性不能覆盖】`
- super的追溯不仅限于直接父类
...
<u>父子类同名this引用【就近原则】 ：通过this找成员变量时，先从当前执行代码的==本类去找成员变量==</u>
...
* **变量前面有this.** 
  * 通过this找成员变量时，先从当前执行代码的==本类去找成员变量==  **【见上面例题】**
  * 如果从当前执行代码的本类中没有找到，会往上找==父类声明的成员变量（==权限修饰符允许在子类中访问的）
...
③ 我们前面讲过，在构造器的首行可以使用"this(形参列表)"，调用本类中重载的构造器，
     结合②，结论：<u>在构造器的首行，"this(形参列表)" 和 "super(形参列表)"只能二选一</u>。

④ <u>如果在子类构造器的首行既没有显示调用"this(形参列表)"，也没有显式调用"super(形参列表)"，</u>
<u>​     则子类此构造器默认调用"super()"，即调用父类中空参的构造器</u>。
...
class Test{
    public static void main(String[] args){
        B b = new B();
        //A类和B类都是默认有一个无参构造，子类B类的默认无参构造中还会默认调用父类A类的默认无参构造
...
>
>  1. 继承中属性不存在覆盖
>  2. 方法存在重写后动态绑定，分析代码时首先确定是执行的方法是属于哪一级父/子？
>  3. `this.属性`调用根据就近原则，通过this找成员变量时，先从当前执行代码的==本类去找成员变量==
>  4. 子类没有重写父类的getInfo方法，返回info字符串的时候，根据就近原则，优先从当前执行代码的本类[Father]去找成员变量【即父类的成员变量】
...
class Son extends Father{
    ...
    public void test(){
        System.out.println(this.getInfo());//属性就近原则，优先从代码执行类中开始找，此处getinfo中  返回的字符串时父类的info串
...
2. 从过程的角度来看：
   ...
   正因为我们调用过子类所有的父类的构造器，所以我们就**会将父类中声明的属性、方法加载到内存中，供子类的对象使用**。
...
Java引用变量有两个类型：`编译时类型`和`运行时类型`。编译时类型由`声明`该变量时使用的类型决定，运行时类型由`实际赋给该变量的对象`决定。简称：**编译时，看左边；运行时，看右边。** **【虚拟方法的调用/非静态的成员方法动态绑定】**
...
多态的适用性：适用于方法，不适用于属性、静态方法属性

> 封装性：
> 	1、权限控制
> 继承性：
> 	1、代码服用
> 	2、方法重写，this.属性是从方法代码执行位置【类中】按就近原则匹配
> 	3、私有属性，会被继承，但是封装性不让访问
> 多态性
> 	1、父类的引用指向子类的对象，编码看左边，运行看右边  原理：非静态成员方法的动态绑定
> 	2、只有非静态成员方法有多态性   属性和静态内容都是跟随类文件的，不具有多态性，也就不需要考虑动态绑定，编译时即可明确调用
> 	3、解耦、抽象
...
class Son extends Person {
    ...
    @Override
    public void show(){
        System.out.println("Son " + this.id);
    }

    public static void main(String[] args) {
        Person person = new Son();
        person.show();//Son 201   此处多态性运行时调用Son类，子类中id = 201
        
        // 如果此处子类没有重写show() 结果： person 101，
        //原因：父类show()中this.id从当前执行代码的Person类中开始寻找【通过this找成员变量时，先从当前执行代码的==本类去找成员变量== 】

        //多态的适用性：适用于方法，不适用于属性
        System.out.println("person.id = " + person.id);//person.id = 101
        System.out.println("( (Son)person).id = " + ((Son) person).id);//( (Son)person).id = 201   强转为子类时
    }
}
```
...
**Java允许程序员不必在编制程序时就确定调用哪一个方法，而是在程序运行的过程中，当方法被调用时，系统根据当时对象本身所属的类来确定调用哪个方法，这种技术被称为后期（动态）绑定**
...
多态：方法 编译看左边，运行看右边 【动态绑定】   属性以左边为准
...
> 3. **动态绑定机制和属性没有关系，子父类存在同名属性时，通过`父类.属性`就是返回父类的属性**；
> 
>    + <u>当子类对象被创建时，在堆中给对象申请内存时，就要看子类和父类都声明了什么实例变量，这些实例变量都要分配内存</u>。
> 
>   + 当子类对象调用方法时，<u>编译器会先在子类模板中看该类是否有这个方法，如果没找到，会看它的父类甚至父类的父类是否声明了这个方法，遵循`从下往上`找的顺序，找到了就停止，一直到根父类都没有找到，就会报编译错误</u>。
> 4. 构造方法，省略this 和 super时默认会调用super()
>5. **用this访问属性和方法时，如果在本类中未找到，会从父类中查找，并且不能违反封装性；**
> 6.   多态分析代码时**变量前面有this.** 【见5.2.2章节案例】     分析：如果父类方法的被子类重写，且被覆盖方法内使用的`this.属性`，this.属性就从子类的成员变量开始寻找；如果没有出现方法的重写，但是该方法内使用的`this.属性`，就从父类的成员变量开始找 【就近原则】
>
>    * 通过this找成员变量时，先从当前执行代码的==本类去找成员变量==  **【见上面例题】**
>   * 如果从当前执行代码的本类中没有找到，会往上找==父类声明的成员变量（==权限修饰符允许在子类中访问的）
> 
...
public class FieldMethodTest {
    public static void main(String[] args){
        ...
        b.display();//20  --子类重写父类方法，运行实际是子类的方法，代码执行类是Sub类，this.count先从子类开始找
...
- 只能比较引用类型，<u>Object类源码中equals()的作用与“==”相同</u>：比较是否指向同一个对象。	 
...
- 当自定义使用equals()时，可以重写【参考String类实现】。用于比较两个对象的“内容”是否都相等
...
System.out.println("hello" == new java.util.Date()); //java: 不可比较的类型: java.lang.String和java.util.Date
```
...
- 在JDK 9中此方法已经被`标记为过时`的。原因：finalize()可能导致内部出现循环引用，导致此对象不能被回收
